...
1. **Default mapping rules** from traditional POS to CGEL categories: `pos-defaults.yaml`. Examples: *determiner/article* → determinative; *conjunction* splits into coordinator, subordinator, and preposition; a sizeable chunk of "adverbs" (*outside*, *abroad*, *ago*, *downstairs*) → intransitive preposition. For *ago*, the Constructionary treats the obligatory measure phrase as a pre-head modifier (compare *ten years before Christ*), rejecting CGEL's complement analysis on p. 632.
