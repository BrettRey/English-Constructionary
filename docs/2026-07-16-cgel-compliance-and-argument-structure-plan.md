...
|---|---|---|---|
| *ago* | Six surfaces call *ten years* an obligatory modifier and *ago* intransitive. | Treat *ten years* as the complement preceding the head *ago*; remove *ago* from the complementless/intransitive examples and add a corrective decision-log entry. | CGEL Ch. 7 §4.2, p. 632 explicitly uses obligatoriness to diagnose complement status. |
...
| Phrase | `phrase-001` omits Clause from the full list and says the immediate head is always a word. | Add Clause: it is a phrase headed by VP with an optional Subject; the VP is ultimately headed by V. Distinguish immediate phrasal heads from ultimate lexical heads throughout. | CGEL Ch. 1, pp. 22–25. |
| Exclamative words | The semantic entry does not record the category split between interrogative and exclamative *what*. | State that exclamative *how* is adverbial, while exclamative *what* is an adjective functioning as external or internal modifier; interrogative *what* is a determinative. | CGEL Ch. 10 §8.1, pp. 918–20. |
...
**Pattern:** `Head:V + Od:NP[theme] + Comp:PP[locative: source/path/goal]`
...
**ID:** `time-away-001`  
**Pattern:** `Head:V[intransitive use] + Od:NP[time-period] + Comp:PP[Head:away]`
