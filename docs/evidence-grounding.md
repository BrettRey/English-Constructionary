...
<!-- SUMMARY: Design for typed evidence links connecting every claim to its ground (CGEL, dictionaries, corpora, papers, judgments) · status: adopted (phases 1-2 built 2026-07-16) · updated: 2026-07-16 -->
...
Source **types**: reference-grammar, dictionary, corpus, treebank, paper, dataset, author-judgment, agent-judgment. Every type is versionable (edition, dump date, commit) so a claim is keyed to what its ground said _then_.
...
**Second typing dimension** — what the evidence grounds, aligned with the ladder: `bears-on: category-assignment | attestation | projection | stability | network | maintenance | control` This is what lets the lint say "this entry claims `secured: maintained` but no evidence link bears on maintenance."
### 3. Attachment points
...
- `agent-judgment` as the _only_ ground for any claim: warn (flag for your adjudication)
...
- **Field name: `grounds`** (Claude's pick, delegated by Brett): avoids collision with the existing free-text `evidence` strings and matches "ground evidence".
- **`agent-judgment` removed**: it can't be checked or replicated (Brett). Agent-drafted claims stay ungrounded until real ground attaches; ungroundedness is itself the flag.
- Phases 1-2 built: registry (`data/sources.yaml`, 13 sources), `grounds` in both schemas (top level, kind rungs, examples, override records), resolution checking in validate, transitional rung lint, and the mechanical retrofit (CGEL page cites, papers, the ago ruling with its cites + contradicts + adjudicates triple).

## Open questions remaining (settle during retrofit)
